...
The instance variable is used to set a VkInstance. A VkInstance is the way your
liblucurious.so connects to the vulkan api.
...
To print a list of available validation layers type B<lucur --pgvl> in your terminal.

=item B<VkExtensionProperties> *ep_instance_props

The ep_instance_props pointer keeps track of all available instance extensions.
To print a list of device instance extensions type B<lucur --pie> in your terminal.
...
To print a list of device extensions type B<lucur --pde VK_QUEUE_GRAPHICS_BIT,1>
in your terminal
...
The device variable is used to store a logical device information.
...
The sc_img_fmt variable stores the surface format return by choose_swap_surface_format().

=item B<VkExtent2D> sc_extent

The sc_extent variable stores the resolution returned by choose_swap_extent() for
