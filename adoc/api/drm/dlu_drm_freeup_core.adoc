...
* **core**: A pointer to the link:../../structs/drm/dlu_drm_core.html[dlu_drm_core] structure.

== DESCRIPTION

The **dlu_drm_freeup_core(3)** function free's up any memory if allocate that belongs to the
link:../../structs/drm/dlu_drm_core.html[dlu_drm_core(3)] structure. It also releases active logind
session on the active KMS node/DRM device.
