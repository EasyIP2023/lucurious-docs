...
The **dlu_data_type** enum constants must be passed to link:../../api/utils/dlu_otba.html[dlu_otba(3)] this choses which link:../../structs/vkcomp/vkcomp.html[vkcomp(3)]
structure member one wants to allocate. You must specify the amount of elements with link:../../structs/utils/dlu_otma_mems.html[dlu_otma_mems(3)]. I wrote this becuase early
on I saw that handles that store references to objects that low-level APIs manipulate could be preallocated and their addresses reused.
